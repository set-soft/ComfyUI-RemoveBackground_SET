...
![Remove Background](doc/Banner.jpg)
...
- &#x0001F4DD; [Usage Notes](#-usage-notes)
   - [Informal explanation of the terms](#informal-explanation-of-the-terms)
...
### Remove background (full)
   ...
   - **Inputs:**
     ...
     - `color` (`STRING`): Color to use when filling the background. You can specify it in multiple ways, even by name, [more here](https://pillow.readthedocs.io/en/stable/reference/ImageColor.html)
...
As mentioned before is hard to compare the models. Some of them might be better for a task others for another.
...
"BiRefNet General" and "InSPyReNet Base 1.2.12" are good alternatives, but much slower and needs more VRAM (upto 4 times).
PDFNet is quite interesting because it performs quite well even when trained with a much smaller dataset, it exploits the DeepAnythingV2 knowledge.
BEN2 is also a good alternative.

But again, this is just a measurement.
...
- These nodes contains the inference code for the models:
  ...
  - [RMFormer](https://github.com/DrowsyMon/RMFormer): Xinhao Deng, Pingping Zhang, Wei Liu, Huchuan Lu
