...
class AttnProcessor2_0:
    ...
    def __call__(
        ...
        **kwargs,
        ...
        # attention_scores = torch.matmul(query, key.transpose(-2, -1))  # QK^T
        # attention_scores = attention_scores / (head_dim ** 0.5)  # 缩放
        # attention_map = F.softmax(attention_scores, dim=-1)  # 计算 attention map
        # print(attention_map.size())
        # print(attention_map.max())
        # print(attention_map.min())
        # m = attention_map[0].unsqueeze(0).mean(dim=1, keepdim=True)
        # m = (m-m.min())/(m.max()-m.min())
        # m = to_pil(m.data.squeeze(1).cpu())
        # m.save('./attnm.png')
        # b=c
