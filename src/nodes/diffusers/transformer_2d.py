...
class Transformer2DModel(nn.Module):
    ...
    def __init__(
        ...
        norm_type: str = "layer_norm",  # 'layer_norm', 'ada_norm', 'ada_norm_zero',
                                        # 'ada_norm_single', 'ada_norm_continuous', 'layer_norm_i2vgen'
        ...
        assert not self.is_input_vectorized
        assert not self.is_input_patches
...
#    def _init_continuous_input(self, norm_type, norm_num_groups, dropout):
...
#         self.out = nn.Linear(self.inner_dim, self.config.num_vector_embeds - 1)
    ...
    def forward(
        ...
        Args:
            hidden_states (`torch.LongTensor` of shape `(batch size, num latent pixels)` if discrete, `torch.Tensor` of shape
                `(batch size, channel, height, width)` if continuous):
                Input `hidden_states`.
