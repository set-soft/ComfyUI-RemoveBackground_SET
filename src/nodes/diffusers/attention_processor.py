...
from .import_utils import is_torch_npu_available, is_xformers_available
        ...
        if use_memory_efficient_attention_xformers:
            if is_added_kv_processor and is_custom_diffusion:
                raise NotImplementedError(
                    "Memory efficient attention is currently not supported for custom diffusion for attention processor "
                    f"type {self.processor}"
                )
            ...
            if is_custom_diffusion:
                ...
                # throw warning
                logger.info(
                    "Memory efficient attention with `xformers` might currently not work correctly if an attention mask is "
                    "required for the attention operation."
                )
    ...
    def forward(
        ...
        **cross_attention_kwargs,
        ...
        if len(unused_kwargs) > 0:
            logger.warning(
                f"cross_attention_kwargs {unused_kwargs} are not expected by {self.processor.__class__.__name__} "
                "and will be ignored."
            )
        cross_attention_kwargs = {
...
class Attention_(nn.Module):
    def __init__(
        ...
        residual_connection: bool = False,
        _from_deprecated_attn_block: bool = False,
        processor=None,  # : Optional["AttnProcessor_"] =
        out_dim: int = None,
    ...
    def set_use_memory_efficient_attention_xformers(
        ...
        is_lora = hasattr(self, "processor") and isinstance(
            self.processor,
            LORA_ATTENTION_PROCESSORS,  # noqa: F821
        )
        is_custom_diffusion = hasattr(self, "processor") and isinstance(
            self.processor,
            (
        ...
        if use_memory_efficient_attention_xformers:
            if is_added_kv_processor and (is_lora or is_custom_diffusion):
                raise NotImplementedError(
                    "Memory efficient attention is currently not supported for LoRA or custom diffusion "
                    f"for attention processor type {self.processor}"
                )
            if not is_xformers_available():
                raise ModuleNotFoundError(
                    (
            ...
            if is_lora:
                ...
                # throw warning
                logger.info(
                    "Memory efficient attention with `xformers` might currently not work correctly if an "
                    "attention mask is required for the attention operation."
                )
                processor = XFormersAttnAddedKVProcessor(attention_op=attention_op)
            else:
                processor = XFormersAttnProcessor(attention_op=attention_op)
    ...
    def get_processor(
        ...
        lora_processor_cls = getattr(
            import_module(__name__), "LoRA" + non_lora_processor_cls_name  # noqa: F821
        )
...
class Attention_mecross(nn.Module):
    ...
    def __init__(
        ...
        if self.added_kv_proj_dim is None and self.only_cross_attention:
            raise ValueError(
                "`only_cross_attention` can only be set to True if `added_kv_proj_dim` is not None. Make "
                "sure to set either `only_cross_attention=False` or define `added_kv_proj_dim`."
            )
        ...
        if processor is None:
            processor = (
                AttnProcessor_mecross(mode=mode)
                if hasattr(F, "scaled_dot_product_attention") and self.scale_qk
                else AttnProcessor()
            )
        self.set_processor(processor)
...
class AttnProcessor_mecross(nn.Module):
    ...
    def __init__(self, mode: Optional[int] = None):
        ...
        # self.conv = nn.Sequential(nn.Conv2d(1280, 1280, kernel_size=3, padding=1), nn.BatchNorm2d(1280), nn.GELU())

        if mode == 4:
            self.mlp_m = nn.Sequential(
                nn.Linear(1280, 1280), nn.GELU(), nn.Linear(1280, 1280)
            )
    ...
    def __call__(
        ...
        **kwargs,
        ...
        if mode == "DBIA":
            query_0, query_1 = torch.chunk(query, dim=0, chunks=2)
...
class PAGJointAttnProcessor2_0:
    ...
    def __call__(
        ...
        # `context` projections.
        encoder_hidden_states_org_query_proj = attn.add_q_proj(
            encoder_hidden_states_org
        )
        encoder_hidden_states_org_key_proj = attn.add_k_proj(encoder_hidden_states_org)
        encoder_hidden_states_org_value_proj = attn.add_v_proj(
            encoder_hidden_states_org
        )
        ...
        if context_input_ndim == 4:
            encoder_hidden_states_org = encoder_hidden_states_org.transpose(
                -1, -2
        ...
        # `context` projections.
        encoder_hidden_states_ptb_query_proj = attn.add_q_proj(
            encoder_hidden_states_ptb
        )
        encoder_hidden_states_ptb_key_proj = attn.add_k_proj(encoder_hidden_states_ptb)
        encoder_hidden_states_ptb_value_proj = attn.add_v_proj(
            encoder_hidden_states_ptb
        )
        ...
        full_mask = torch.zeros(
            (seq_len, seq_len), device=query_ptb.device, dtype=query_ptb.dtype
        )
        ...
        full_mask = full_mask.unsqueeze(0).unsqueeze(
            0
        )  # Add batch and num_heads dimensions
        ...
        if context_input_ndim == 4:
            encoder_hidden_states_ptb = encoder_hidden_states_ptb.transpose(
                -1, -2
            ).reshape(batch_size, channel, height, width)
...
class PAGCFGJointAttnProcessor2_0:
    ...
    def __call__(
        ...
        **kwargs,
        ...
        # chunk
        hidden_states_uncond, hidden_states_org, hidden_states_ptb = (
        ...
        encoder_hidden_states_org = torch.cat(
            [encoder_hidden_states_uncond, encoder_hidden_states_org]
        )
...
class StableAudioAttnProcessor2_0:
    ...
    def apply_partial_rotary_emb(
        ...
        x_rotated = apply_rotary_emb(
            x_to_rotate, freqs_cis, use_real=True, use_real_unbind_dim=-2
        )
    ...
    def __call__(
        ...
        if rotary_emb is not None:
            ...
            query_rotated = apply_rotary_emb(
                query_to_rotate, rotary_emb, use_real=True, use_real_unbind_dim=-2
            )
            ...
            if not attn.is_cross_attention:
                ...
                key_rotated = apply_rotary_emb(
                    key_to_rotate, rotary_emb, use_real=True, use_real_unbind_dim=-2
                )
    ...
    def __call__(
        ...
        if key_rotary_emb is None:
            ...
            if base_sequence_length is not None:
                softmax_scale = (
                    math.sqrt(math.log(sequence_length, base_sequence_length))
                    * attn.scale
                )
            else:
